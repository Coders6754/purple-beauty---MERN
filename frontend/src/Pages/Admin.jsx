...
const Admin = () => {
  ...
  const getUser = async (page, qu = "", qa = "") => {
    ...
    let res = await fetch(
      `${BackendURL}/user/?page=${page}&limit=5&name=${qu}&address=${qa}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  async function addProductFunc() {
    let res = await fetch(`${BackendURL}/user/addProduct`, {
      ...
      body: JSON.stringify({
        image_link: localStorage.getItem("cloudinary"),
  ...
  return (
    <>
      ...
      <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
        {loading && <Spinner color="red" size={"lg"} />}
      </Box>
      {error && (
        <Alert status="error">
          <AlertIcon />
      ...
      <Heading textAlign={"center"} m={"2% 0"} fontSize={"23px"}>
        Add Product
      </Heading>
      <Box
        w={["87%", "87%", "30%"]}
      ...
      >
        <Input
          w={{ base: "15%", sm: "31%", lg: "15%" }}
          placeholder="Search Username"
          onChange={handleFilter}
        ></Input>
        <Input
          w={{ base: "15%", sm: "30%", lg: "15%" }}
      ...
      >
        ...
        <Button variant={"outline"} color="red" isDisabled={true}>
          {page}
        </Button>
        <Button
          variant={"outline"}
